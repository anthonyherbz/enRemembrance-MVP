			...
			template = (
				<div className={styles.cover}>
					<Image
						layout='fill'
						objectFit='contain'
						src={currPage.quadrants[0].content}
					/>
				</div>
			);
			break;
			...
			{page !== 0 ? (
				<div className={styles.buttons}>
					{currPage.templateName != "uninitialized" ? (
						<button
							onClick={() => updateTemplateJson("uninitialized")}
						>
							Undo Choice
						</button>
					) : null}
					{currPage.templateName == "uninitialized" ? (
						<>
							<button
								onClick={() => updateTemplateJson("splitTop")}
							>
								Select split top
							</button>
							<button
								onClick={() =>
									updateTemplateJson("splitBottom")
								}
							>
								Select split bottom
							</button>
							<button
								onClick={() => updateTemplateJson("splitFour")}
							>
