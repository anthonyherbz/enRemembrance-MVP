 ...
 */ function createErrorHandler(/**
   * Used for debugging
   */ _source, capturedErrors) {
    return (err)=>{
        var ref;
        if (// TODO-APP: Handle redirect throw
        err.digest !== _hooksServerContext.DYNAMIC_ERROR_CODE && err.digest !== _notFound.NOT_FOUND_ERROR_CODE && !((ref = err.digest) == null ? void 0 : ref.startsWith(_redirect.REDIRECT_ERROR_CODE))) {
        ...
        if (staticGenerationStore && isStaticGeneration) {
            if (opts && typeof opts === "object") {
                if (opts.cache === "no-store") {
                    ...
                    throw new DynamicServerError(`no-store fetch ${url}${pathname ? ` ${pathname}` : ""}`);
                }
                ...
                if (typeof next.revalidate === "number" && (typeof fetchRevalidate === "undefined" || next.revalidate < fetchRevalidate)) {
                    ...
                    // TODO: ensure this error isn't logged to the user
                    // seems it's slipping through currently
                    throw new DynamicServerError(`revalidate: ${next.revalidate} fetch ${url}${pathname ? ` ${pathname}` : ""}`);
                }
 ...
 */ function useFlightResponse(writable, req, serverComponentManifest, rscChunks, flightResponseRef, nonce) {
    ...
    const { createFromReadableStream ,  } = require("next/dist/compiled/react-server-dom-webpack");
...
async function renderToString(element) {
    if (!_utils.shouldUseReactRoot) return ReactDOMServer.renderToString(element);
...
async function renderToHTMLOrFlight(req, res, pathname, query, renderOpts) {
   ...
   */ const isStaticGeneration = renderOpts.supportsDynamicHTML !== true;
    ...
    const htmlRendererErrorHandler = createErrorHandler("htmlRenderer", capturedErrors);
    const { buildManifest , subresourceIntegrityManifest , serverComponentManifest , serverCSSManifest ={} , ComponentMod , dev , fontLoaderManifest ,  } = renderOpts;
     ...
     */ const ServerComponentsRenderer = createServerComponentRenderer(()=>{
        ...
        if (isStaticGeneration) {
            ...
            renderOpts.revalidate = typeof (staticGenerationStore == null ? void 0 : staticGenerationStore.revalidate) === "undefined" ? defaultRevalidate : staticGenerationStore == null ? void 0 : staticGenerationStore.revalidate;
