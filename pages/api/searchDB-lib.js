...
export default async function getServerSideProps(req, res) {
	...
	try {
		...
		if (type == "tag") {
			querySql =
				"SELECT story_tags.story_id, story_tags.tag_string, stories.id AS story_id, stories.title, CONVERT(stories.create_date, char) AS create_date, users.handle, users.id AS user_id FROM story_tags LEFT JOIN stories ON story_tags.story_id = stories.id LEFT JOIN users ON users.id = stories.author_id WHERE story_tags.tag_string LIKE ?"
		}
